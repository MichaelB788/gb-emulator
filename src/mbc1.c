...
void set_rom_bank_number(struct cartridge *cart, uint8_t val) {
...
void set_ram_bank_number(struct cartridge *cart, uint8_t val) {
  if (cart->ram_size >= KiB_32 || cart->rom_size >= MiB_1) {
    cart->mbc1.ram_bank = val & 0x3;
  }
}

void mbc1_write_rom(struct cartridge *cart, const uint16_t addr,
                    const uint8_t val) {
...
uint8_t mbc1_read_ram(const struct cartridge *cart, const uint16_t addr) {
...
void mbc1_write_ram(struct cartridge *cart, const uint16_t addr,
                    const uint8_t val) {
  if (cart->mbc1.ram_enabled) {
    const size_t internal_address =
        ((addr - 0xA000) + (cart->mbc1.ram_bank * KiB_8)) % cart->ram_size;
