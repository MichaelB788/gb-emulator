...
void impl_add(struct cpu *cpu, uint8_t operand) {
  ...
  cpu->F &= ~FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (A & 0xF) + (operand & 0xF) > 0xF);
  cpu_set_flag_as(cpu, FLAG_C, sum > 0xFF);
...
void impl_adc(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F &= ~FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (A & 0xF) + (operand & 0xF) + carry > 0xF);
  cpu_set_flag_as(cpu, FLAG_C, sum > 0xFF);
...
void impl_sub(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F |= FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (A & 0xF) < (operand & 0xF));
  cpu_set_flag_as(cpu, FLAG_C, A < operand);
...
void impl_sbc(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F |= FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (A & 0xF) < (operand & 0xF) + carry);
  cpu_set_flag_as(cpu, FLAG_C, A < (uint16_t)(operand + carry));
...
void impl_cp(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, A - operand == 0);
  cpu->F |= FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (A & 0xF) < (operand & 0xF));
  cpu_set_flag_as(cpu, FLAG_C, A < operand);
}
...
uint8_t impl_inc_u8(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F &= ~FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (operand & 0xF) == 0xF);
...
uint8_t impl_dec_u8(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F |= FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (operand & 0xF) == 0x0);
...
void impl_add_r16(struct cpu *cpu, uint16_t operand) {
  ...
  cpu->F &= ~FLAG_N;
  cpu_set_flag_as(cpu, FLAG_H, (HL & 0xFFF) + (operand & 0xFFF) > 0xFFF);
  cpu_set_flag_as(cpu, FLAG_C, sum > 0xFFFF);
...
uint16_t impl_add_sp_e8(struct cpu *cpu, int8_t e8) {
  ...
  cpu->F &= ~(FLAG_Z | FLAG_N);
  cpu_set_flag_as(cpu, FLAG_H, (SP & 0xF) + (e8 & 0xF) > 0xF);
  cpu_set_flag_as(cpu, FLAG_C, (SP & 0xFF) + (e8 & 0xFF) > 0xFF);
...
void impl_and(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, cpu->A == 0);
  cpu->F &= ~(FLAG_N | FLAG_C);
  cpu->F |= FLAG_H;
}
...
void impl_bit_b3(struct cpu *cpu, uint8_t b3, uint8_t r8) {
  cpu_set_flag_as(cpu, FLAG_Z, (r8 & 1 << b3) == 0);
  cpu->F &= ~FLAG_N;
  cpu->F |= FLAG_H;
}
...
uint8_t impl_rl(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F &= ~(FLAG_N | FLAG_H);
  cpu_set_flag_as(cpu, FLAG_C, (operand & 0x80) != 0);
...
uint8_t impl_rr(struct cpu *cpu, uint8_t operand) {
  ...
  cpu_set_flag_as(cpu, FLAG_Z, result == 0);
  cpu->F &= ~(FLAG_N | FLAG_H);
  cpu_set_flag_as(cpu, FLAG_C, (operand & 1) != 0);
