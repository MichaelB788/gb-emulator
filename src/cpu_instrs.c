...
void ld_r8_u8(struct cpu *cpu) {
  cpu->r8[Y_BIT_FIELD(cpu->opcode)] = cpu_fetch_u8(cpu);
}

void ld_r16_u16(struct cpu *cpu) { cpu_set_r16(cpu, cpu_fetch_u16(cpu)); }

void ld_hl_ind_r8(struct cpu *cpu) {
  cpu_write_hl(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]);
}

void ld_hl_ind_u8(struct cpu *cpu) { cpu_write_hl(cpu, cpu_fetch_u8(cpu)); }
...
void ld_u16_ind_a(struct cpu *cpu) {
  cpu_write_byte(cpu, cpu_fetch_u16(cpu), cpu->A);
}

void ldh_u8_ind_a(struct cpu *cpu) {
  cpu_write_byte(cpu, (0xFF00 | cpu_fetch_u8(cpu)), cpu->A);
}
...
void ld_a_u16_ind(struct cpu *cpu) {
  cpu->A = cpu_read_byte(cpu, cpu_fetch_u16(cpu));
}

void ldh_a_u8_ind(struct cpu *cpu) {
  cpu->A = cpu_read_byte(cpu, (0xFF00 | cpu_fetch_u8(cpu)));
}
...
void adc_hl_ind(struct cpu *cpu) { impl_adc(cpu, cpu_read_hl(cpu)); }

void adc_u8(struct cpu *cpu) { impl_adc(cpu, cpu_fetch_u8(cpu)); }

void add_r8(struct cpu *cpu) {
  impl_add(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]);
}

void add_hl_ind(struct cpu *cpu) { impl_add(cpu, cpu_read_hl(cpu)); }

void add_u8(struct cpu *cpu) { impl_add(cpu, cpu_fetch_u8(cpu)); }

void sbc_r8(struct cpu *cpu) {
  impl_sbc(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]);
}

void sbc_hl_ind(struct cpu *cpu) { impl_sbc(cpu, cpu_read_hl(cpu)); }

void sbc_u8(struct cpu *cpu) { impl_sbc(cpu, cpu_fetch_u8(cpu)); }

void sub_r8(struct cpu *cpu) {
  impl_sub(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]);
}

void sub_hl_ind(struct cpu *cpu) { impl_sub(cpu, cpu_read_hl(cpu)); }

void sub_u8(struct cpu *cpu) { impl_sub(cpu, cpu_fetch_u8(cpu)); }

void cp_r8(struct cpu *cpu) { impl_cp(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]); }

void cp_hl_ind(struct cpu *cpu) { impl_cp(cpu, cpu_read_hl(cpu)); }

void cp_u8(struct cpu *cpu) { impl_cp(cpu, cpu_fetch_u8(cpu)); }
...
void and_hl_ind(struct cpu *cpu) { impl_and(cpu, cpu_read_hl(cpu)); }

void and_u8(struct cpu *cpu) { impl_and(cpu, cpu_fetch_u8(cpu)); }

void or_r8(struct cpu *cpu) { impl_or(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]); }

void or_hl_ind(struct cpu *cpu) { impl_or(cpu, cpu_read_hl(cpu)); }

void or_u8(struct cpu *cpu) { impl_or(cpu, cpu_fetch_u8(cpu)); }

void xor_r8(struct cpu *cpu) {
  impl_xor(cpu, cpu->r8[Z_BIT_FIELD(cpu->opcode)]);
}

void xor_hl_ind(struct cpu *cpu) { impl_xor(cpu, cpu_read_hl(cpu)); }

void xor_u8(struct cpu *cpu) { impl_xor(cpu, cpu_fetch_u8(cpu)); }
...
void call_a16(struct cpu *cpu) { cpu_call_a16(cpu, cpu_fetch_u16(cpu), true); }

void call_cc_a16(struct cpu *cpu) {
  cpu_call_a16(cpu, cpu_fetch_u16(cpu), cpu_test_cond(cpu));
}

void jp_hl(struct cpu *cpu) { cpu->PC = cpu_get_hl(cpu); }

void jp_a16(struct cpu *cpu) { cpu_jump_a16(cpu, cpu_fetch_u16(cpu), true); }

void jp_cc_a16(struct cpu *cpu) {
  cpu_jump_a16(cpu, cpu_fetch_u16(cpu), cpu_test_cond(cpu));
}

void jr_e8(struct cpu *cpu) {
  const int8_t offset = (int8_t)cpu_fetch_u8(cpu);
...
void rst_vec(struct cpu *cpu) {
  cpu_call_a16(cpu, (uint16_t)(cpu->opcode & 0x38), true);
}
...
void ld_u16_ind_sp(struct cpu *cpu) {
  const uint16_t addr = cpu_fetch_u16(cpu);
  cpu_write_byte(cpu, addr, cpu->SP & 0xFF);
  cpu_write_byte(cpu, addr + 1, cpu->SP >> 8);
}
...
void pop_r16stk(struct cpu *cpu) { cpu_set_r16stk(cpu, cpu_pop_u16(cpu)); }

void push_r16stk(struct cpu *cpu) {
  bus_tick(cpu->bus); // Internal tick
  cpu_push_u16(cpu, cpu_get_r16stk(cpu));
}
...
void halt(struct cpu *cpu) { cpu->state = CPU_HALTED; }
