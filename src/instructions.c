...
int ld_r8_r8(struct gameboy *gb) {
  const reg_t dest = field_y(gb->opcode);
  const reg_t src = field_z(gb->opcode);
...
int ld_r16_n16(struct gameboy *gb) {
  const regpair_t dest = field_y(gb->opcode) & 0b110;
  set_regpair(&gb->cpu, dest, fetch_n16(gb));
  return 12;
}

int ld_hl_ind_r8(struct gameboy *gb) {
  const reg_t dest = field_z(gb->opcode);
...
int ld_r16_ind_a(struct gameboy *gb) {
  const regpair_t src = field_y(gb->opcode) & 0b110;
...
int adc_r8(struct gameboy *gb) {
  const reg_t reg = field_z(gb->opcode);
